...
class ApiService {
  ...
  static String? get syncToken => _inMemoryToken;
      ...
      try {
        _dio.httpClientAdapter = IOHttpClientAdapter(
          createHttpClient: () {
            ...
            // Allow locally generated certificates strictly during Wi-Fi LAN testing
  ...
  static String formatNetworkError(dynamic error) {
    if (error is DioException) {
      ...
      if (errStr.contains('113') || errStr.contains('no route to host')) {
        return "🌐 Wi-Fi Router block or IP change detected (errno 113).\n👉 Verify your Laptop IP in General Settings.";
      }
      if (error.type == DioExceptionType.connectionError || errStr.contains('connection refused')) {
        return "🚫 Server unreachable at target URL.\n👉 Make sure laptop backend is running and smartphone is connected to the same Wi-Fi / Hotspot.";
      }
      if (error.type == DioExceptionType.connectionTimeout || error.type == DioExceptionType.receiveTimeout) {
        return "⏱️ Connection timed out. Check Wi-Fi connection and firewall settings.";
      }
